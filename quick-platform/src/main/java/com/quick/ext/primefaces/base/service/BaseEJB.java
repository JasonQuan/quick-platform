...
public abstract class BaseEJB<T extends AbstractEntity, E extends AbstractEntity> implements Serializable {
    ...
    /**
     * get int ((Long) q.getSingleResult()).byteValue(); get other ...
     ...
     * @param jpql jpql muse end with where condation, eg: 'select o from entity
     * o where 1=1 '
     * @param columnModels
     ...
     * @param jpql the 'FROM' in jpql must be upper case, and jpql must be end
     * with a blank space
     * @param jpqlParametersKeyValue map<key,value> for jpql setParameter(key,
     * value)
     * @return
     */
